...
export class WorkspaceTextWindowComponent implements OnInit, OnDestroy {
    ...
    const promise = new Promise<TAnnotation>((resolve, reject) => {
      this.annotationService.createAnnotation(annotation).pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          this.messageService.add(this.msgConfService.generateErrorMessageConfig(`Saving annotation failed: ${JSON.parse(error.error)['message']}`));
          reject(error);
  ...
  async onSaveAnnotationFeatures(featuresList: { feature: TFeature, value: string }[]) {
    ...
    forkJoin([...newFeaturesObs, ...updateFeaturesObs]).pipe(
      takeUntil(this.unsubscribe$),
      catchError((error: HttpErrorResponse) => {
        this.messageService.add(this.msgConfService.generateErrorMessageConfig(`Saving features failed: ${JSON.parse(error.error)['message']}`));
        return throwError(() => new Error(error.error));
      }),
    ).subscribe(() => {
  ...
  loadData() {
    ...
    forkJoin([
      ...
      takeUntil(this.unsubscribe$),
      catchError((error: HttpErrorResponse) => {
        this.messageService.add(this.msgConfService.generateErrorMessageConfig(`Loading data failed: ${JSON.parse(error.error)['message']}`));
