  ...
  layer$ = this.retrieveLayerById.pipe(
    switchMap(layerId => this.layerService.retrieveLayerById(layerId).pipe(
      catchError((error: HttpErrorResponse) => {
        this.messageService.add(this.msgConfService.generateErrorMessageConfig(`Error retrieving layer: ${JSON.parse(error.error)['message']}`));
  ...
  layers$ = merge(
    ...
    this.addLayer.pipe(
      switchMap(newLayer => this.layerService.createLayer(newLayer).pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          this.messageService.add(this.msgConfService.generateWarningMessageConfig(`Adding layer "${newLayer.name}" failed: ${JSON.parse(error.error)['message']}`));
          return throwError(() => new Error(error.error));
        }),
      )),
    ...
    this.removeLayer.pipe(
      switchMap(layerId => this.layerService.removeLayerById(layerId).pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          this.messageService.add(this.msgConfService.generateWarningMessageConfig(`Removing layer failed: ${JSON.parse(error.error)['message']}`));
          return throwError(() => new Error(error.error));
        }),
      )),
      switchMap(() => this.layerService.retrieveLayerList()),
    ...
    this.updateLayer.pipe(
      switchMap(updatedLayer => this.layerService.updateLayerById(updatedLayer).pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          this.messageService.add(this.msgConfService.generateWarningMessageConfig(`Updating layer "${updatedLayer.name}" failed: ${JSON.parse(error.error)['message']}`));
          return throwError(() => new Error(error.error));
        }),
      )),
      switchMap(() => this.layerService.retrieveLayerList()),
  ...
  features$ = merge(
    this.retrieveLayerFeatures.pipe(
      switchMap(layerId => this.layerService.retrieveLayerFeatureList(layerId).pipe(
        catchError((error: HttpErrorResponse) => {
          this.messageService.add(this.msgConfService.generateWarningMessageConfig(`Retrieving layer features failed: ${JSON.parse(error.error)['message']}`));
          return throwError(() => new Error(error.error));
        }),
      )),
    ),
    this.addFeature.pipe(
      switchMap(newFeature => this.featureService.createFeature(newFeature).pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          this.messageService.add(this.msgConfService.generateWarningMessageConfig(`Adding feature "${newFeature.name}" failed: ${JSON.parse(error.error)['message']}`));
          return throwError(() => new Error(error.error));
        }),
      )),
    ...
    this.updateFeature.pipe(
      switchMap(updatedFeature => this.featureService.updateFeatureById(updatedFeature).pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          this.messageService.add(this.msgConfService.generateWarningMessageConfig(`Updating feature "${updatedFeature.name}" failed: ${JSON.parse(error.error)['message']}`));
          return throwError(() => new Error(error.error));
        }),
      )),
    ...
    this.removeFeature.pipe(
      switchMap(removedFeature => this.featureService.removeFeatureById(removedFeature.id!).pipe(
        ...
        catchError((error: HttpErrorResponse) => {
          this.messageService.add(this.msgConfService.generateWarningMessageConfig(`Removing feature "${removedFeature.name}" failed: ${JSON.parse(error.error)['message']}`));
          return throwError(() => new Error(error.error));
        }),
      )),
