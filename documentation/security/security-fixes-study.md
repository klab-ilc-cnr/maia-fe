...
|---|---|---|---|---|
| Setup | `a13f07f3` | `proxy.conf.json` → `https://192.168.92.24:11443` (`/api /lexo /texto /cash`, `secure:false`) | config | S (~5 min) |
...
| F4 | `410a80b9` | **quill 1.3.7 → 2.0.3** (usato solo dal `p-editor` di primeng; nessuna import diretta nel codice) | bump | M (~6 min) |
...
**Totale effort pratico ≈ 1 ora** di lavoro (setup escluso), 7 commit di fix + 1 setup. Nessun adattamento di business-code richiesto (i fix runtime erano bump compatibili; l'unica modifica di config codice è `skipLibCheck`).
...
| `quill` 2.0.3 | low | nuova advisory XSS su export HTML `=2.0.3`; **2.0.4 non ancora pubblicato** | monitorare (appena esce, bump) |
...
| Smoke test completo con backend (VM `192.168.92.24:11443`) | M |
...
5. Nessun file di business-code modificato per la sicurezza: impatto su funzionalità atteso **nullo**; da confermare con smoke test (login + editor ricco `p-editor`/quill 2 + export PDF).
...
5. Tenere monitorata la coda Dependabot (nuova advisory quill in attesa di 2.0.4); con toolchain moderna i fix sono point-release banali.
